...
// issues counts grouped by priority
...
$query = "
        SELECT count(*) as the_count, priority
        FROM $mantis_bug_table
        WHERE $specific_where
        AND date_submitted >= " . $db->qstr( $db_datetimes['start'] ) . "
        AND date_submitted <= " . $db->qstr( $db_datetimes['finish'] ) . "
...
foreach ($result as $row) {
    $issues_fetch_from_db_chart[$row['priority']] = $row['the_count'];
}


// building tables
function data_table ($type) {
	global $status_values, $resolved_status_threshold, $status_enum_string, $priority_enum_string, $issues_fetch_from_db;
	...
	$data_table_print .= "<table class='width100' cellspacing='1'><tr><td width='100%' class='form-title'>" . lang_get( 'plugin_MantisStats_by_priority_long' ) . "</td>";
	...
	foreach ($issues_fetch_from_db as $key => $val) {
		...
		$table_rows[$key] = "<tr><td>" . MantisEnum::getLabel($priority_enum_string, $key) . "</td>";
...
foreach ($issues_fetch_from_db_chart as $key => $val) {
	$chart_data .= "<set value='" . $val . "' label='" . number_format( $val ) . " " . htmlspecialchars( MantisEnum::getLabel( $priority_enum_string, $key) , ENT_QUOTES ) . "' />";
}
...
<div id="wrapper">
    ...
    <div id="databox">
        ...
        <div id="titleText">
            <div id="title"><?php echo lang_get( 'plugin_MantisStats_by_priority_long' ); ?></div>
...
<?php if ( $render_print_js != "no_render" ) { ?>
        ...
        <p class="space30Before" />
        <strong><?php echo lang_get( 'plugin_MantisStats_by_priority_chart' ); ?></strong>
