...
// needed arrays
$top_issue_with_notes = $average_notes = $without_notes = array();


// top issue with most notes [open|resolved]
$query = "
...
if ( isset( $result[0] ) ) {
    $row = $result[0];
    $top_issue_with_notes['open'] = $row['the_count'];
} else { $top_issue_with_notes['open'] = 0; }
...
if ( isset( $result[0] ) ) {
    $row = $result[0];
    $top_issue_with_notes['resolved'] = $row['the_count'];
} else { $top_issue_with_notes['resolved'] = 0; }
...
// average N. of notes in issues [open|resolved|all]
// N. of cases without notes [open|resolved|all]
$query = "
        SELECT count(distinct mbt.id) as issues_count, count(*) as notes_count
...
if ( $number_of_issues['open'] ) {
    $average_notes['open'] = round( $row['notes_count'] / $number_of_issues['open'], 2 );
} else { $average_notes['open'] = 0; }

$without_notes['open'] = $number_of_issues['open'] - $row['issues_count'];
$tmp = $row['notes_count'];
...
if ( $number_of_issues['resolved'] ) {
    $average_notes['resolved'] = round( $row['notes_count'] / $number_of_issues['resolved'], 2 );
} else { $average_notes['resolved'] = 0; }

$without_notes['resolved'] = $number_of_issues['resolved'] - $row['issues_count'];

if ( $number_of_issues['all'] ) {
    $average_notes['all'] = round( ($tmp + $row['notes_count']) / $number_of_issues['all'], 2 );
} else { $average_notes['all'] = 0; }

$without_notes['all'] = $without_notes['open'] + $without_notes['resolved'];
...
<tr>
<td width='100%' class='form-title'><?php echo lang_get( 'plugin_MantisStats_notes_stats' ); ?></td>
...
<td>" . lang_get( 'plugin_MantisStats_mostnt_in_single' ) . "</td>
<td class='right'>" . number_format( $top_issue_with_notes['open'] ) . "</td>
<td class='right'>" . number_format( $top_issue_with_notes['resolved'] ) . "</td>
<td class='right'>" . number_format( max( $top_issue_with_notes['open'], $top_issue_with_notes['resolved'] ) ) . "</td>
</tr>

<tr " . helper_alternate_class() . ">
<td>" . lang_get( 'plugin_MantisStats_average_nt_periss' ) . "</td>
<td class='right'>" . $average_notes['open'] . "</td>
<td class='right'>" . $average_notes['resolved'] . "</td>
<td class='right'>" . $average_notes['all'] . "</td>
</tr>

<tr " . helper_alternate_class() . ">
<td>" . lang_get( 'plugin_MantisStats_no_iss_without_nt' ) . "</td>
<td class='right'>" . number_format( $without_notes['open'] ) . "</td>
<td class='right'>" . number_format( $without_notes['resolved'] ) . "</td>
<td class='right'>" . number_format( $without_notes['all'] ) . "</td>
</tr>
</table>
...
$query = "
        SELECT mbt.id, count(*) as the_count, mbt.status, mbt.summary
        FROM $mantis_bug_table mbt
        LEFT JOIN $mantis_bugnote_table mbnt
        ON mbt.id = mbnt.bug_id
        WHERE $specific_where
...
function toplist ($type) {
    ...
    $out = pagination( count( $summaries ), $offset_switch, plugin_page( 'issues_by_notes' ) );
    ...
    if ( count( $summaries ) > 0 ) {
        foreach ( $summaries as $key => $val ) {
            ...
            <td class='right'>" . number_format( $counts[$key] ) . "</td>
            </tr>";
        }
    }
...
$top_list_header = "
    <table class='width100' cellspacing='1'>
    <tr>
    <td width='100%' class='form-title'>Issue summary</td>
    <td class='heading'>" . lang_get( 'plugin_MantisStats_no_of_notes' ) . "</td>
    </tr>
";
...
<div id="wrapper">
    ...
    <div id="databox">
        ...
        <p class="space30Before" />
        <strong><?php echo lang_get( 'plugin_MantisStats_iss_open_top_nt' ); ?><sup>&Dagger;</sup></strong>
        <p />
        <?php echo $top_list_header . toplist('open') . "</table>"; ?>

        <p class="space30Before" />
        <strong><?php echo lang_get( 'plugin_MantisStats_iss_res_top_nt' ); ?><sup>&Dagger;</sup></strong>
        <p />
